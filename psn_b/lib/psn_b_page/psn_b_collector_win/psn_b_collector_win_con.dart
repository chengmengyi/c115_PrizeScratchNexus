...
class PsnBCollectorWinCon extends PsnRootCon implements PlayListener{
  ...
  _initContentList(){
    ...
    if(PsnBValueUtils.instance.getCollectorPoint1()){
      ...
      contentList.addAll(List.filled(3, PsnBContentBean(content: first, reward: PsnBValueUtils.instance.getReward(), win: true)));
    }
    ...
    while (contentList.length < 20) {
      ...
      if (counts[pick]! <= 2) {
        contentList.add(PsnBContentBean(content: pick, reward: PsnBValueUtils.instance.getReward(), win: false));
      } else {
        counts[pick] = 2;
      }
    }
